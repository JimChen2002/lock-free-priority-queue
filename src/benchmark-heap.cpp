...
/**
 * Benchmark file for fine-grained locking version of priority queue
 * 
...
void work_debug(int id) {
    ...
    elem = __sync_fetch_and_add(&elem, 1);
    pq->insert((int)elem, (int)elem, id);
}
...
run(void *_args)
{
    ...
    args->measure = cnt;
    cout << pq->time_elapsed.count() / 1e6  << "ns\n";
    return NULL;
}
